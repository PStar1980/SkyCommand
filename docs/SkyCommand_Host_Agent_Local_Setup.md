...
# Host Agent preflight/readiness heartbeat freshness threshold (15-600 seconds).
SKYCOMMAND_HOST_AGENT_HEALTH_FRESHNESS_SECONDS=60
...
Before installing the task, stop any manually running `npm run host-agent` process; the installer refuses to create a second host agent. The task starts after the current Windows user logs on, runs with limited privileges, prevents duplicate task instances, and retries startup for up to an hour if Docker/Temporal is still coming online. The task deliberately retains Task Scheduler's **Interactive** logon type so host-side Git operations continue to use the signed-in user's normal Git/network credential context. To prevent that interactive security context from producing a visible console, Task Scheduler launches `wscript.exe` and `Start-SkyCommandHostAgentHidden.vbs`; the GUI-subsystem launcher starts the PowerShell -> Node worker chain with window style `0`/hidden before a console surface is created. The existing PowerShell console-hide guard remains as defense in depth. Runtime output is retained under `logs/host-agent/scheduled-task.log` with a small rollover guard. The hidden PowerShell runner also records its current PID in `logs/host-agent/scheduled-task.runner.pid`; the lifecycle helper validates that PID against the expected SkyCommand runner command line before using it to terminate the complete PowerShell -> Node process tree. The Windows runner deliberately allows native Node stderr while the worker is active because Temporal writes normal lifecycle diagnostics to stderr; the native process exit code remains authoritative. `npm run host-agent:auto-start:status` reports Task Scheduler state, the registered logon type, and the detected host-agent process state and exits non-zero when the automatic worker is not actually running. The task deliberately remains outside Docker so host-owned Git worktrees are never writable by Linux containers. Remove it with `npm run host-agent:auto-start:uninstall`; manual `start` and `stop` helpers are also available.

> **Why SkyCommand does not use S4U here:** Windows S4U tasks intentionally do not receive normal network credentials. The Host Agent must perform guarded Git operations against remote origins, so moving the worker into S4U would trade the visible-terminal problem for broken authenticated network behavior. The hidden `wscript.exe` launcher keeps the correct interactive-token security context while removing the console UI. After upgrading, run `npm run host-agent:auto-start:install` once so the existing task action is replaced, then prove the real path with `npm run host-agent:check` and one guarded host Git operation such as Local Dev Pull or Local Repository Sync.
...
The Host Agent is deliberately outside `docker compose`. Restarting or rebuilding the Docker stack does not install a host process. After automatic startup has been installed, keep using `npm run host-agent:auto-start:start` and `npm run host-agent:auto-start:stop` as the public lifecycle commands. `start` is duplicate-safe and refuses to create a second scheduled worker when the validated runner is already active. `stop` first stops the Task Scheduler launcher, then validates the recorded SkyCommand runner PID and terminates that runner's complete child process tree so the hidden Node Host Agent cannot survive as an orphan. Use `npm run host-agent:auto-start:status` to confirm the resulting state. If the status reports `FAILED` or `STOPPED`, inspect `logs/host-agent/scheduled-task.log` before Development Promotion. Manual `npm run host-agent` remains the direct diagnostic path, but do not run manual and scheduled-task instances together. Re-run `npm run host-agent:check` after any restart to prove the complete Docker Temporal → host activity bridge.

The Host Agent writes its liveness into `worker.temporal_worker_heartbeats` with role metadata `HOST_AGENT`, so the control plane can distinguish it from the Docker Temporal worker while retaining one worker-health model. Heartbeats use a short-lived PostgreSQL connection with a bounded connection/query timeout so a Docker PostgreSQL restart cannot permanently strand the telemetry path; failures are retried on the normal heartbeat cadence and recovery is logged once connectivity returns. Command Center surfaces the persisted state as its own **Host Agent** availability card, including disabled, online, stale, and offline states. The effective `SKYCOMMAND_HOST_AGENT_HEALTH_FRESHNESS_SECONDS` value is reported in Host Agent availability evidence; values outside the bounded 15-600 second range fall back to the safe default of 60 seconds. Workflow launch safety is slightly stronger than the dashboard signal: stale/missing heartbeat telemetry falls back to the live Temporal probe before host execution is declared unavailable.
