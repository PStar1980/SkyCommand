...
**Active status:** Phase 15 is in progress. The Add Tool lane now preserves the repository's normal package shape: administrators choose any new package directory inside `packages/`, define a `src/...` entrypoint that retains the uploaded filename, and centralize optional schemas under `packages/tools/contracts`. Onboarding descriptors are analyzed but not retained. Warnings remain advisory, and preview/SHA-256 values remain registration evidence only. Phase 15.6 verification infrastructure is present in Manage Tools, including non-executing contract checks, disabled-tool controlled test runs, and explicit enablement. The PostgreSQL comparison tool is now registered disabled, and the Add Tool handoff opens its verification panel directly through verification focus mode. The remaining proof is contract checking, controlled execution, enablement, Run Tools, and workflow routing.
