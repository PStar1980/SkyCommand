...
**Active status:** Phase 15 is in progress. Phase 15.2 now delivers the first functional Tool Catalogue administration surface: **Tools > Manage Tools** can list, inspect, create, edit, enable, and disable PostgreSQL catalogue records, including positional parameters, static choices, visibility, permission, risk, runtime, repository-relative script paths, and structured-output metadata. Dedicated `ADMIN_TOOL_READ` / `ADMIN_TOOL_WRITE` permissions and audited transactional API services protect the new writes. Phase 15.3 is next: designate exactly one repository as the SkyCommand repository and resolve its active-profile path before trusted upload staging and managed file registration begin. PostgreSQL remains the runtime authority; no upload execution, dependency installation, automatic Git commit, or sandbox claim is introduced by Phase 15.2.
...
| Phase 15   | 🟡 In progress | Assisted Tool Catalogue administration: architecture/authoring kit and Manage Tools catalogue CRUD are complete; next designate one SkyCommand repository and active-profile root, then add trusted Node.js upload staging, static validation, optional descriptor/schema prefilling, disabled-first file promotion and registration, contract-check proof, and audit history                                                                                                                                                                                                                                                                                                                                                           |
