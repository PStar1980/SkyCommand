...
**Active status:** Phase 15 is in progress. Phase 15.3 now designates a single active repository as the trusted **SkyCommand repository**, resolves the current configuration profile through `core.repository_paths`, verifies repository and managed-tools filesystem readiness, and exposes clear readiness/error evidence on **Configuration > Repositories**. The database enforces one designation, designated repositories cannot be disabled until the role is cleared, and the Admin API exposes audited set/clear operations plus reusable readiness enforcement for the upcoming onboarding service. Phase 15.4 is next: trusted upload staging and static analysis for one Node.js script plus optional descriptor and output schema. PostgreSQL remains the runtime authority; no uploaded code is executed, no dependency is installed, and no Git commit is performed by Phase 15.3.
...
Phase 15 adds the **assisted Tool Catalogue administration layer** without reopening the Phase 14 runtime. The Manage Tools page now provides administrative CRUD services for tool identity, category, runtime, permission, risk, visibility, positional parameters, static options, script path, status, and structured-output metadata. Configuration > Repositories now also manages the single SkyCommand repository designation and displays active-profile readiness for its physical root and `packages/tools/custom` managed destination. The onboarding assistant next adds trusted Node.js upload staging, static syntax and metadata analysis, an optional `skycommand.tool.json` descriptor for form prefilling, optional output-schema validation, a preview of final repository paths and database records, disabled-first registration, and audited file promotion. The descriptor and schemas remain onboarding/reporting assets only - the PostgreSQL catalogue continues to decide what may run.
...
| Phase 15   | 🟡 In progress | Assisted Tool Catalogue administration: architecture/authoring kit, Manage Tools catalogue CRUD, and single SkyCommand repository designation with active-profile filesystem readiness are complete; next add trusted Node.js upload staging, static validation, optional descriptor/schema prefilling, disabled-first file promotion and registration, contract-check proof, and audit history                                                                                                                                                                      |
