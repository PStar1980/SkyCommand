...
The ingestion and data-contract foundation is portable beyond the original macroeconomic use case while retaining the established FRED, Bank of Canada, and Statistics Canada paths. **Phase 17 is now active**, extending SkyCommand from a Dockerized application into a guarded infrastructure control surface. The current slices add a Kubernetes-ready provider seam, Host Agent Docker inventory, Engine/Compose/container/image/volume/network observability, and permission-separated Compose project Start/Stop/Restart controls for externally managed discovered projects. SkyCommand's own Compose project remains protected from synchronous self-termination so the control plane cannot destroy the API/Temporal path that is executing and auditing the action.
