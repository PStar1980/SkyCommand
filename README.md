...
The ingestion and data-contract foundation is portable beyond the original macroeconomic use case while retaining the established FRED, Bank of Canada, and Statistics Canada paths. **Phase 17 is now active**, extending SkyCommand from a Dockerized application into a guarded infrastructure control surface. The first slice adds a Kubernetes-ready provider seam, read-only Docker inventory, Host Agent transport, Docker navigation, and operator-facing Engine/Compose/container/image/volume/network observability before any write controls are enabled.
...
| Phase 17 | 🔄 In progress | Harden and operationalize the Dockerized SkyCommand stack. The read-only Docker control-plane foundation now routes inventory through the Host Agent, exposes Engine/Compose/container/image/volume/network observability in Admin-Web, and establishes a provider seam designed to accept Kubernetes later; lifecycle controls, streamed telemetry, and audited write operations remain next. |
