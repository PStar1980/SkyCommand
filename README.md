...
**Active status:** Phase 15 is in progress. Phase 15.5 now completes the managed-registration lane in **Tools > Add Tool**. A clean 24-hour staging session becomes editable catalogue configuration, an exact server-resolved command/file/PostgreSQL preview, and a fingerprint-protected disabled-first registration. SkyCommand generates the canonical descriptor, writes a hidden package inside the designated repository, creates tool/visibility/parameter/option records disabled, atomically promotes the package into `packages/tools/custom/<toolCode>`, verifies final hashes, records audit evidence, and removes the temporary session. Uploaded code is still not executed, dependencies are not installed, and no Git operation occurs. Phase 15.6 is next: contract check, optional controlled execution, Run Tools proof, and workflow routing proof.
...
Phase 15 adds the **assisted Tool Catalogue administration layer** without reopening the Phase 14 runtime. **Tools > Manage Tools** provides catalogue CRUD for identity, category, runtime, permission, risk, visibility, positional parameters, static options, script path, status, and structured-output metadata. **Configuration > Repositories** manages the single SkyCommand repository designation and active-profile readiness for its physical root and `packages/tools/custom` destination. **Tools > Add Tool** stages and statically analyzes one trusted Node.js script plus optional descriptor/schema, then provides editable prefill, a fingerprinted command/file/database preview, canonical descriptor generation, disabled-first PostgreSQL registration, atomic package promotion, final hash verification, and audit/recovery evidence. Descriptors, schemas, and hashes remain onboarding/reporting evidence only - PostgreSQL continues to decide what may run, and the uploaded code is not executed during Phase 15.5.
...
| Phase 15   | 🟡 In progress | Assisted Tool Catalogue administration: architecture/authoring kit, Manage Tools CRUD, single SkyCommand repository readiness, trusted upload/static analysis, editable prefill, fingerprinted preview, disabled-first registration, canonical descriptor generation, and atomic managed-file promotion are complete; next add contract-check proof, controlled execution, Run Tools/workflow validation, and closure                                                                                                                                                |
