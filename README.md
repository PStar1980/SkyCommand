...
The ingestion and data-contract foundation is portable beyond the original macroeconomic use case while retaining the established FRED, Bank of Canada, and Statistics Canada paths. **Phase 17 is now active**, extending SkyCommand from a Dockerized application into a guarded infrastructure control surface. The current slices add a Kubernetes-ready provider seam, Host Agent Docker inventory, Engine/Compose/container/image/volume/network observability, permission-separated Compose and container lifecycle controls, deep container inspection with bounded logs, live Docker event and resource-telemetry lanes, project-focused application-stack observability, and deep image/volume/network inspection with attachment-aware cleanup policy. Docker cleanup is separated from ordinary lifecycle control: only SUPER_ADMIN receives the cleanup permission by default, image/network removal is live-usage-checked and audited, Docker built-in networks are protected, and persistent volume deletion is intentionally not exposed. SkyCommand's own Compose project and containers remain protected from synchronous self-termination so the control plane cannot destroy the API/Temporal path that is executing and auditing an action.
