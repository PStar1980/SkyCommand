...
**Active status:** Phase 15 is in progress. Phase 15.5.1 keeps the completed Add Tool registration lane flexible and accessible: the administrator may choose any new package directory inside `packages/` (the default remains `packages/tools/custom/<toolCode>`), warnings are advisory, and preview/SHA-256 values are temporary registration evidence only. They are not read by CLI, Run Tools, workers, schedules, or workflows and cannot lock out an edited registered tool. Registration still creates a disabled PostgreSQL record, atomically promotes the package, and never executes uploaded code, installs dependencies, or performs Git operations. Phase 15.6 is next: contract check, optional controlled execution, Run Tools proof, and workflow routing proof.
...
| Phase 15   | 🟡 In progress | Assisted Tool Catalogue administration: architecture/authoring kit, Manage Tools CRUD, repository readiness, trusted upload/static analysis, editable prefill, flexible package destinations, advisory preview evidence, disabled-first registration, canonical descriptor generation, and atomic package promotion are complete; next add contract-check proof, controlled execution, Run Tools/workflow validation, and closure                                                                                                                                    |
