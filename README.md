...
**Active status:** Phase 15 is in progress. Phase 15.4 now adds **Tools > Add Tool**, where a trusted administrator can stage one Node.js/CommonJS entry script plus an optional `skycommand.tool.json` descriptor and optional output schema. SkyCommand verifies repository readiness, stores the files in a random 24-hour non-executable session under `logs/tool-onboarding`, parses JavaScript syntax without execution, validates descriptor/schema rules and active catalogue references, detects missing dependencies and destination/tool-code collisions, and presents confidence-labelled suggestions plus audit-safe file hashes. Phase 15.5 is next: editable prefill, final repository/database preview, disabled-first registration, and managed-file promotion. PostgreSQL remains the runtime authority; Phase 15.4 performs no uploaded-code execution, dependency installation, catalogue registration, managed-package write, or Git commit.
...
Phase 15 adds the **assisted Tool Catalogue administration layer** without reopening the Phase 14 runtime. **Tools > Manage Tools** provides catalogue CRUD for identity, category, runtime, permission, risk, visibility, positional parameters, static options, script path, status, and structured-output metadata. **Configuration > Repositories** manages the single SkyCommand repository designation and active-profile readiness for its physical root and `packages/tools/custom` destination. **Tools > Add Tool** now stages and statically analyzes one trusted Node.js script plus optional descriptor/schema, producing syntax, dependency, policy, schema, collision, and catalogue-reference findings without executing the source. The next increment converts a clean session into editable prefilled configuration, destination/database preview, disabled-first registration, and audited file promotion. Descriptors, schemas, and hashes remain onboarding/reporting evidence only - PostgreSQL continues to decide what may run.
...
| Phase 15   | 🟡 In progress | Assisted Tool Catalogue administration: architecture/authoring kit, Manage Tools CRUD, single SkyCommand repository readiness, and trusted Node.js upload staging/static analysis are complete; next add editable prefill, destination/database preview, disabled-first file promotion and registration, contract-check proof, and end-to-end audit history                                                                                                                                                                                                          |
