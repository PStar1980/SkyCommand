...
**Active status:** Phase 15 is in progress. The Add Tool lane preserves the repository's normal package shape: administrators choose any new package directory inside `packages/`, define a `src/...` entrypoint that retains the uploaded filename, and centralize optional schemas under `packages/tools/contracts`. Onboarding descriptors are analyzed but not retained. Warnings remain advisory, and preview/SHA-256 values remain registration evidence only. Phase 15.6 verification is now proven end to end: the PostgreSQL comparison tool passed its non-executing contract check, completed a disabled controlled run, and was explicitly enabled. Database Build now emits `database_build_summary.v1`, giving the upcoming database-build comparison workflow structured build evidence before it evaluates `database_health_summary.v1` and `postgresql_database_comparison_summary.v1`. The remaining work is Run Tools/workflow proof, the custom workflow summary, and Phase 15 closure.
...
| Phase 15   | 🟡 In progress | Assisted Tool Catalogue administration: architecture/authoring kit, Manage Tools CRUD, repository readiness, trusted upload/static analysis, editable prefill, flexible package roots, preserved `src/...` entrypoints, disposable descriptors, centralized contract promotion/reuse, disabled-first registration, contract checking, controlled disabled-tool execution, and explicit enablement are proven. PostgreSQL comparison is registered and enabled; Database Build emits `database_build_summary.v1`; workflow tool nodes bypass interactive confirmation while retaining permissions and risk authorization. Run Tools/workflow comparison proof, custom summary, and closure remain. |
