...
### Accepted tool manifest snapshots

Phase 14.6 persists the repository contract that SkyCommand has explicitly accepted for each manifest-backed tool. Migration `00063__tool_manifest_registration_snapshots.sql` adds versioned historical snapshots containing the normalized manifest, source repository, manifest path, runtime, entrypoint, semantic output type, validator version, validation status, and SHA-256 hashes for the manifest, entrypoint, output schema, and contract sample.

Use the snapshot commands after applying the migration:
...
`preview` performs no writes. `sync` accepts the currently validated repository contracts and preserves the previous snapshot as `SUPERSEDED` when a new manifest or entrypoint is accepted. `check` compares the live repository files with the current database snapshot and persists `VALID` or `DRIFTED` status.

Manifest-backed API and worker execution requires a current `VALID` snapshot. A missing snapshot fails with `TOOL_MANIFEST_SNAPSHOT_REQUIRED`; changed files or contract metadata fail with `TOOL_MANIFEST_SNAPSHOT_DRIFT`. Tool execution metadata retains the accepted snapshot ID and hashes so Tool History can prove which contract authorized the run. Production Readiness includes a Tool Manifest Registration section covering schema presence, registered tools, initial snapshot registration, and live drift.
...
### Tool manifests and contract checks

Phase 14.4 introduces repository manifests under `packages/ingestion/manifests/<toolCode>/skycommand.tool.json`. A manifest declares the runtime entrypoint, parameter types and bindings, permissions, timeout and execution channels, semantic output type, output JSON Schema, and a non-destructive sample result. Repository-relative path validation blocks traversal and symlink escape; SHA-256 hashes are calculated for the normalized manifest, entrypoint, output schema, and contract sample so later registration can detect drift.
...
```bash
node packages/ingestion/src/loadFREDMacroData.js --skycommand-describe
node packages/ingestion/src/loadFREDMacroData.js --skycommand-contract-check
```
...
| Phase 14   | 🔄 In Progress | Structured Tool Results and workflow output contracts: one generic process adapter, separate log/result channels, versioned extensible `ToolResult` envelopes, canonical inline/Temporal workflow paths, normalized FRED/BoC/StatCan summaries, repository map/package proofs via `repo_map_generate` and `repo_zip_generate`, manifest-backed Dev Commit separation, strict `skycommand.tool.json` and output-schema validation, non-destructive contract checks, durable accepted-manifest snapshots, runtime hash enforcement, condition-path enforcement, scheduled-result evidence, and purpose-built tool/summary rendering |
