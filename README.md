...
**Active status:** Phase 14 is now in progress. Phase 14.1 established the universal structured-tool-result transport and generic child-process adapter. Phase 14.2 delivers the first workflow-native proof case: FRED now emits `macro_ingestion_summary.v1`, loader statistics are returned as deliberate objects while the existing console transcript remains intact, Workflow History renders compact run totals plus an indicator result table, and workflow conditions can consume stable fields such as `nodes.fred_ingestion.output.totals.rowsInserted`. Phase 13 is complete and remains the live telemetry, runtime context, parameterized workflow, condition, summary, and visual execution foundation that Phase 14 now strengthens. Phase 14.2.1 hardens the development runtime by preventing nodemon from treating ephemeral ToolResult transport files as source changes. Phase 14.2.2 aligns durable workflow output persistence with semantic result contracts and prevents successful side-effecting tools from being rerun when only ledger persistence fails.
