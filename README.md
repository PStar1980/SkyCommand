...
The ingestion and data-contract foundation is portable beyond the original macroeconomic use case while retaining the established FRED, Bank of Canada, and Statistics Canada paths. **Phase 17 is complete**, extending SkyCommand from a Dockerized application into a guarded Docker infrastructure control plane. The finished Docker domain includes a Kubernetes-ready provider/target seam, Host Agent inventory, Engine/Compose/container/image/volume/network observability, permission-separated lifecycle and cleanup controls, deep container/resource inspection, bounded logs, native Docker events, live CPU/memory/I/O telemetry, application-stack aggregation, durable Docker Operations audit evidence, control-plane self-protection, and explicit stale/error recovery semantics. Read, lifecycle control, and cleanup remain separate privileges; cleanup defaults to SUPER_ADMIN only, persistent-volume deletion is intentionally not exposed, and raw Docker daemon/shell access never reaches the browser or API. Phase 17.9 also distinguishes Host Agent transport health from Docker provider health, preserves last-known live evidence when a source becomes stale, and consolidates the Docker regression contract under `npm run docker-integration:self-test`.

SkyCommand's control plane is now fully Dockerized: **PostgreSQL**, **Temporal**, **SkyCommand Temporal worker**, **scheduler/listener Node worker**, **SkyCommand API**, and the production-style **Admin-Web** NGINX service all run through the persistent Compose stack. Host-run Vite remains the development/HMR lane. The one intentional host-native component is the narrowly scoped **SkyCommand Host Agent**, which owns operations that must touch Windows-managed resources such as active Git worktrees. It polls a dedicated Temporal activity queue, exposes no host shell or HTTP endpoint, records liveness into the shared worker heartbeat model, and can start automatically at Windows logon. Workflow preflight fails closed before execution when a required Host Agent is disabled or unavailable.
...
# After the manual proof, let Windows start the host-native agent automatically at logon
...
The SkyCommand Host Agent is intentionally **not** another container. It runs on the repository-owning host, connects outbound to Temporal at `localhost:7233`, and polls the dedicated `skycommand-host-local` activity queue for durable allow-listed host operations. Phase 17 also lets the same host-native process inspect Docker images, volumes, and networks through normalized resource contracts, perform narrowly guarded unused image/network cleanup, observe Docker container lifecycle events, and sample bounded Docker resource statistics. Live event/CPU/memory/I/O/PID metadata is relayed outbound to the localhost-only SkyCommand API using the existing internal-service token; the API then exposes authenticated Server-Sent Events streams to Admin-Web. Docker-side `local_repo_sync` executions still use the short Temporal bridge workflow, while live Docker events and telemetry deliberately avoid Temporal history. No host HTTP listener or arbitrary shell endpoint is exposed. The Host Agent refuses `DOCKER_LOCAL`, validates the configured host repository profile before polling, emits Temporal worker heartbeats, and retains the same guarded Git/Docker control boundaries. Command Center shows Host Agent availability separately from the Docker Temporal worker, and Docker surfaces distinguish Host Agent transport health from Docker Engine/provider health; live lanes carry normalized source error codes and retain last-known samples/events with explicit stale/degraded messaging while automatic retry continues. Workflow start preflight blocks host-targeted workflows before queueing when dispatch is disabled or the Host Agent heartbeat is stale/offline. On Windows, the optional `host-agent:auto-start:*` helpers install a limited-privilege per-user Scheduled Task so the agent comes back automatically after logon without moving host mutation rights into Docker. See `docs/SkyCommand_Host_Agent_Local_Setup.md` for host setup and `docs/SkyCommand_Docker_Infrastructure_Control_Plane.md` for the completed Docker control/observability architecture.
