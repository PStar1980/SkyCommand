...
**Active status:** Phase 15 is in progress. Phase 14 completed the workflow-native `ToolResult` architecture and its eight-node Development Promotion proof. Phase 15.1 now establishes the assisted Tool Catalogue administration plan, a self-contained authoring guide, an AI build prompt, and a working managed-tool template. The first functional increment will add Tools > Manage Tools so administrators can create and maintain PostgreSQL catalogue configuration without manual SQL; later increments add one designated SkyCommand repository, trusted upload staging, static validation, optional output-schema analysis, controlled file promotion, and registration. PostgreSQL remains the runtime authority, and the initial onboarding scope is intentionally conservative: Node.js/CommonJS, one entry script, positional parameters, no automatic dependency installation, no automatic Git commit, and no claim that uploaded code is sandboxed.
...
Phase 15 adds the **assisted Tool Catalogue administration layer** without reopening the Phase 14 runtime. The minimum product is a Manage Tools page backed by administrative CRUD services for tool identity, category, runtime, permission, risk, visibility, positional parameters, static options, script path, status, and structured-output metadata. The onboarding assistant then adds a trusted Node.js upload path: static syntax and metadata analysis, an optional `skycommand.tool.json` descriptor for form prefilling, optional output-schema validation, a preview of final repository paths and database records, disabled-first registration, and audited file promotion under `packages/tools/custom/<toolCode>/`. Exactly one repository will be designated as the SkyCommand repository through Configuration > Repositories; its active profile path supplies the physical root. The descriptor and schemas remain onboarding/reporting assets only - the PostgreSQL catalogue continues to decide what may run.
...
| Phase 15   | 🟡 In progress | Assisted Tool Catalogue administration: approved architecture and authoring kit; next add catalogue CRUD and Tools > Manage Tools, then one designated SkyCommand repository, trusted Node.js upload staging, static validation, optional descriptor/schema prefilling, disabled-first file promotion and registration, contract-check proof, and audit history                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
