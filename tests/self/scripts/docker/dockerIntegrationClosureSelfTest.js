...
assert.match(
  readme,
  /\| Phase 17\s+\| ✅\s+\| Operate Docker infrastructure with guarded controls, deep observability, diagnostics, and durable audit evidence\. \|/,
);
assert.match(readme, /Phase 17 provider\/target boundary intentionally leaves room/);
assert.match(readme, /SkyCommand_Docker_Infrastructure_Control_Plane\.md/);
assert.match(readme, /future \*\*Kubernetes\*\* sibling provider/);
