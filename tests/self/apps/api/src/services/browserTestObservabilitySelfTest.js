...
assert.ok(playwrightConfig.includes("args: ['--start-maximized']"));
assert.ok(playwrightConfig.includes('viewport: null'));
assert.ok(playwrightConfig.includes("browserName: 'chromium'"));
assert.ok(playwrightConfig.includes('SKYCOMMAND_BROWSER_VIEWPORT_WIDTH'));
assert.ok(playwrightConfig.includes('SKYCOMMAND_BROWSER_VIEWPORT_HEIGHT'));
assert.ok(workflowSpec.includes('SKYCOMMAND_BROWSER_INTERACTIVE_HOLD_MS'));
assert.ok(presenterScript.includes('SW_MAXIMIZE = 3'));
assert.ok(presenterScript.includes('FocusDurationMs'));
