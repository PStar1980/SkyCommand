...
assert.ok(css.includes('--sky-sidebar-width: 25rem;'), 'Desktop sidebar should be widened for PLAYWRIGHT AUTOMATION.');
...
assert.ok(page.includes('sky-form-control'), 'Automation Registry filters must use SkyCommand dark form controls.');
assert.ok(page.includes('table-hover sky-table sky-canonical-operations-table'), 'Automation Registry must use the canonical dark selectable table treatment.');
assert.ok(page.includes('sky-table sky-detail-table'), 'Registry detail must use the SkyCommand dark detail table treatment.');
assert.ok(page.includes('sky-btn-ghost'), 'Clear filters should use the standard secondary SkyCommand action style.');
