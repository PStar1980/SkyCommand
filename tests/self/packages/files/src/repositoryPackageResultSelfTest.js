...
async function run() {
  ...
  try {
    ...
    assert.strictEqual(parsed.includeTests, false);
    ...
    assert.strictEqual(includeTestsParsed.includeTests, true);
    const includeTestFiles = flattenFiles(
      scanDirectory(sourceRoot, sourceRoot, includeTestsParsed),
    );
    assert.ok(
      includeTestFiles.some((file) =>
        file.relativePath.endsWith(path.join('tests', 'self', 'sampleSelfTest.js')),
      ),
    );
