...
function run() {
  const outputSchema = JSON.parse(
    fs.readFileSync(
      path.resolve(sourceDir, '../../tools/contracts/git_branch_sync_summary.v1.schema.json'),
      'utf8',
    ),
  );
  ...
  const mainMergeSource = fs.readFileSync(path.resolve(sourceDir, 'main_merge.js'), 'utf8');
