...
function run() {
  const outputSchema = JSON.parse(
    fs.readFileSync(
      path.resolve(sourceDir, '../../tools/contracts/git_local_sync_summary.v1.schema.json'),
      'utf8',
    ),
  );
  ...
  const source = fs.readFileSync(path.resolve(sourceDir, 'local_repo_sync.js'), 'utf8');
