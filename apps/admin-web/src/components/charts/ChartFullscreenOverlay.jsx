...
const OVERLAY_GEOMETRY_SCALE = 1.22;
...
function cloneAndScaleChartOption(value, scale = OVERLAY_GEOMETRY_SCALE) {
  const cloneValue = (nestedValue) => {
    ...
    for (const [key, childValue] of Object.entries(nestedValue)) {
      if (typeof childValue === 'number' && ['symbolSize', 'barWidth', 'borderWidth'].includes(key)) {
  ...
  const normalizeGrid = (grid) => ({
    ...
    bottom: typeof grid.bottom === 'number' ? Math.max(grid.bottom, 48) : grid.bottom,
    left: typeof grid.left === 'number' ? Math.max(grid.left, 34) : grid.left,
    right: typeof grid.right === 'number' ? Math.max(grid.right, 44) : grid.right,
  });
