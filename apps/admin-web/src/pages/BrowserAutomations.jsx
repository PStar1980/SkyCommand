...
function emptyForm(options = {}) {
  ...
  return {
    ...
    requiresConfirmation: false, confirmationText: '', assistantEnabled: false, sideEffectLevel: 'READ_ONLY', idempotencyMode: 'READ_ONLY',
...
function formFromAutomation(automation, options = {}) {
  ...
  return {
    ...
    permissionCode: automation.permissionCode || 'BROWSER_AUTOMATION_RUN', riskCode: automation.riskCode || 'low', requiresConfirmation: Boolean(automation.requiresConfirmation), confirmationText: automation.confirmationText || '', assistantEnabled: Boolean(automation.assistantEnabled),
...
function buildPayload(form) {
  return {
    ...
    permissionCode: form.permissionCode || null, riskCode: form.riskCode, requiresConfirmation: Boolean(form.requiresConfirmation), confirmationText: form.requiresConfirmation ? form.confirmationText.trim() || null : null, assistantEnabled: Boolean(form.assistantEnabled),
...
function AutomationRegistryForm({ canEditCode = true, form, onChange, options, saving = false, onSubmit, submitLabel }) {
  ...
  const setSideEffects = (level) => onChange((current) => ({ ...current, sideEffectLevel: level, idempotencyMode: level === 'READ_ONLY' ? 'READ_ONLY' : current.idempotencyMode === 'READ_ONLY' ? 'IDEMPOTENT' : current.idempotencyMode, requiresConfirmation: level === 'HIGH_IMPACT' ? true : current.requiresConfirmation, assistantEnabled: level === 'HIGH_IMPACT' ? false : current.assistantEnabled }));
  const setConfirmationRequired = (required) => onChange((current) => ({ ...current, requiresConfirmation: required, assistantEnabled: required ? false : current.assistantEnabled }));
  ...
  return <form className="sky-card-body" onSubmit={onSubmit}>
    <div className="row g-3">
      ...
      <div className="col-12"><div className="d-flex flex-wrap gap-4"><div className="form-check form-switch"><input checked={form.requiresConfirmation} className="form-check-input" id="automationRequiresConfirmation" onChange={(event) => setConfirmationRequired(event.target.checked)} type="checkbox" /><label className="form-check-label" htmlFor="automationRequiresConfirmation">Confirmation required</label></div><div className="form-check form-switch"><input checked={form.assistantEnabled} className="form-check-input" disabled={form.requiresConfirmation || !form.permissionCode} id="automationAssistantEnabled" onChange={(event) => update('assistantEnabled', event.target.checked)} type="checkbox" /><label className="form-check-label" htmlFor="automationAssistantEnabled">Assistant execution enabled</label></div><div className="form-check form-switch"><input checked={form.enabled} className="form-check-input" id="automationEnabled" onChange={(event) => update('enabled', event.target.checked)} type="checkbox" /><label className="form-check-label" htmlFor="automationEnabled">Enabled</label></div></div><div className="form-text sky-muted mt-2">Assistant execution is explicit opt-in, headless-only, permission-scoped, and unavailable for confirmation-required automations.</div></div>
