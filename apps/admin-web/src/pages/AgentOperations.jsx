        ...
        <div className="col-xl-7">
          ...
          >
            {detailLoading ? <div className="sky-empty-state py-4">Loading evidence…</div> : !detail ? <div className="sky-empty-state py-4">Select an Agent Run to inspect its durable evidence.</div> : <div className="sky-card-body">
              ...
              <div className="mt-3"><div className="sky-page-kicker mb-2">Managed Capability Effects</div>{(detail.capabilityEffects || []).length === 0 ? <div className="sky-empty-state py-3">No managed capability effect was requested by this run.</div> : <div className="table-responsive"><table className="table table-sm sky-table"><thead><tr><th>Capability / decision</th><th>Credential lifecycle evidence</th><th>Native Browser identity</th><th>Dispatch / outcome</th></tr></thead><tbody>{(detail.capabilityEffects || []).map((effect) => <tr key={effect.effectId}><td><div className="fw-semibold">{effect.capabilityKind || 'UNKNOWN'} · {effect.capabilityCode || 'UNKNOWN'}</div><div className="small sky-muted">{effect.authorityDecision || 'UNKNOWN'} · surface {effect.executionSurfaceDecision?.mode || 'UNKNOWN'}</div><div className="small sky-mono">{effect.effectId || '—'}</div></td><td><span className="sky-pill sky-pill-info">State: {effect.managedCredential?.state || 'UNKNOWN'}</span><div className="small sky-mono">Grant: {effect.managedCredential?.reference || 'ABSENT'}</div><div className="small sky-mono">Fingerprint: {effect.managedCredential?.credentialFingerprint || 'ABSENT'}</div><div className="small">Audience: {effect.managedCredential?.audience || 'ABSENT'}</div><div className="small">Issued: {formatDate(effect.managedCredential?.issuedAt)} · Expires: {formatDate(effect.managedCredential?.expiresAt)}</div><div className="small">Closed: {formatDate(effect.managedCredential?.closedAt)} · Epoch: {effect.managedCredential?.revocationEpoch ?? '—'}</div>{effect.managedCredential?.statusReason ? <div className="small sky-muted">Reason: {effect.managedCredential.statusReason}</div> : null}</td><td className="sky-mono">{effect.nativeBrowserWorkflowId || '—'}{effect.browserAutomationRunId ? <><br /><span className="small">run {effect.browserAutomationRunId}</span></> : null}{effect.nativeBrowserWorkflowId ? <div><a href={`/browser-automations/operations?workflowId=${encodeURIComponent(effect.nativeBrowserWorkflowId)}`}>Open Browser Operations</a></div> : null}</td><td><StatusPill status={effect.dispatchState || 'UNKNOWN'} /><div className="small">{effect.outcomeCertainty || 'UNKNOWN'}</div>{effect.denialReason ? <div className="small text-danger">{effect.denialReason}</div> : null}{effect.reconciliation?.reconciledWorkflowId ? <div className="small sky-muted">Reconciled {effect.reconciliation.reconciledWorkflowId}</div> : null}</td></tr>)}</tbody></table></div>}</div>
