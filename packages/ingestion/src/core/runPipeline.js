...
function materializeItemAttempts(requestAttempts = [], finalResult = {}) {
  ...
  const finalAttempt = {
    ...
    revisionsDetected: finalResult.revisionsDetected || 0,
    qualityIssueCount: finalResult.qualityIssueCount || 0,
    qualityStatusCode: finalResult.qualityStatusCode || 'PASS',
    revisionEvents: Array.isArray(finalResult.revisionEvents) ? finalResult.revisionEvents : [],
    rejectionEvents: Array.isArray(finalResult.rejectionEvents) ? finalResult.rejectionEvents : [],
    qualityIssues: Array.isArray(finalResult.qualityIssues) ? finalResult.qualityIssues : [],
...
async function runPipelineItem({
  ...
  try {
    ...
    const result = {
      ...
      revisionsDetected: loadResult?.revisionsDetected || 0,
      qualityIssueCount: loadResult?.qualityIssueCount || 0,
      qualityStatusCode: loadResult?.qualityStatusCode || 'PASS',
      revisionEvents: Array.isArray(loadResult?.revisionEvents) ? loadResult.revisionEvents : [],
      rejectionEvents: Array.isArray(loadResult?.rejectionEvents) ? loadResult.rejectionEvents : [],
      qualityIssues: Array.isArray(loadResult?.qualityIssues) ? loadResult.qualityIssues : [],
    ...
    const result = {
      ...
      indicatorCode: code,
      outcome: 'FAILED',
      stagingRows: evidence.stagingRows || 0,
      stagingMinDate: evidence.stagingMinDate || null,
      stagingMaxDate: evidence.stagingMaxDate || null,
      sourceMaxDate: evidence.stagingMaxDate || null,
      previousTargetMaxDate: evidence.previousTargetMaxDate || null,
      newRowsDetected: evidence.newRowsDetected || 0,
      rowsInserted: evidence.rowsInserted || 0,
      rowsUpdated: evidence.rowsUpdated || 0,
      rowsUnchanged: evidence.rowsUnchanged || 0,
      rowsRejected: evidence.rowsRejected || 0,
      revisionsDetected: evidence.revisionsDetected || 0,
      qualityIssueCount: evidence.qualityIssueCount || 0,
      qualityStatusCode: evidence.qualityStatusCode || 'FAIL',
      revisionEvents: Array.isArray(evidence.revisionEvents) ? evidence.revisionEvents : [],
      rejectionEvents: Array.isArray(evidence.rejectionEvents) ? evidence.rejectionEvents : [],
      qualityIssues: Array.isArray(evidence.qualityIssues) ? evidence.qualityIssues : [],
      currentTargetMaxDate: evidence.currentTargetMaxDate || null,
      startedAt: startedAt.toISOString(),
      finishedAt: completedAt.toISOString(),
      completedAt: completedAt.toISOString(),
      durationMs: completedAt.getTime() - startedAt.getTime(),
      errorCategoryCode: error.errorCategoryCode || requestAttempts.at(-1)?.errorCategoryCode || null,
