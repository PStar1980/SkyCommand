...
INSERT INTO core.config_profiles (profile_code, profile_name, description, active)
VALUES ('DOCKER_LOCAL','Docker Local Development','Local Docker worker profile mounted beneath /workspace/SkyEco System.',TRUE)
ON CONFLICT (profile_code) DO UPDATE
SET profile_name = EXCLUDED.profile_name,
    description = EXCLUDED.description,
    active = EXCLUDED.active;
...
WITH source_paths AS (
  SELECT rp.repo_id, rp.active, replace(rp.root_path, E'\\', '/') AS normalized_path
  ...
  SELECT
    repo_id,
    active,
    '/workspace' || substring(normalized_path FROM position('/skyeco system/' IN lower(normalized_path))) AS root_path
  FROM source_paths
  WHERE position('/skyeco system/' IN lower(normalized_path)) > 0
)
INSERT INTO core.repository_paths (repo_id, profile_id, root_path, active)
SELECT translated.repo_id, docker_profile.profile_id, translated.root_path, translated.active
...
FROM (
  VALUES
    ...
    ('data_ingestion_tools','ingestion_fred','loadFREDMacroData','Run FRED Ingestion','Loads active FRED macroeconomic indicators into PostgreSQL with optional selected indicators and batched concurrency.','packages/ingestion/src/loadFREDMacroData.js','node','INGESTION_RUN_FRED','medium',TRUE,'This will call the FRED ingestion pipeline and write any new data into PostgreSQL.',TRUE,TRUE,10),
    ('data_ingestion_tools','ingestion_boc','loadBoCMacroData','Run Bank of Canada Ingestion','Loads active Bank of Canada macroeconomic indicators into PostgreSQL with optional selected indicators and batched concurrency.','packages/ingestion/src/loadBoCMacroData.js','node','INGESTION_RUN_BOC','medium',TRUE,'This will call the Bank of Canada ingestion pipeline and write any new data into PostgreSQL.',TRUE,TRUE,20),
    ('data_ingestion_tools','ingestion_statcan','loadStatCanMacroData','Run Statistics Canada Ingestion','Loads active Statistics Canada vector-based macroeconomic indicators into PostgreSQL with optional selected indicators and batched concurrency.','packages/ingestion/src/loadStatCanMacroData.js','node','INGESTION_RUN_STATCAN','medium',TRUE,'This will call the Statistics Canada ingestion pipeline and write any new data into PostgreSQL.',TRUE,TRUE,30),
...
JOIN (
  VALUES
    ...
    ('git_repo_status','cli'), ('git_repo_status','admin-web'), ('git_repo_status','api'), ('git_repo_status','worker'),
...
JOIN (
  VALUES
    ...
    ('ingestion_fred','indicators','Indicators','string','Optional FRED indicators, comma/space/newline separated. Leave blank for every configured FRED indicator.',FALSE,NULL,NULL,10),
    ('ingestion_fred','concurrency','Concurrency','number','Optional batch concurrency. Defaults to 3 and caps at 10.',FALSE,'3',NULL,20),
    ('ingestion_boc','indicators','Indicators','string','Optional Bank of Canada indicator codes, comma/space/newline separated. Leave blank for every configured BoC indicator.',FALSE,NULL,NULL,10),
