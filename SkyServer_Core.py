# ╔═══════════════════════════════════════════════════════════════════════╗
# ║  SkyServer_Core.py                                                    ║
# ║  Core utilities shared by SkyServer, SkyScheduler, and SkyServer_Logs ║
# ║  - No circular imports (this module imports nothing from others)      ║
# ║  - Safe config loading (absolute path + .env + env overrides)         ║
# ║  - Path helpers, atomic writers, JSONL helpers                        ║
# ║  - Date/time helpers & heartbeat                                      ║
# ║  - Optional: log detection & empty-aggregate guard                    ║
# ╚═══════════════════════════════════════════════════════════════════════╝
...
def load_config() -> Dict[str, Any]:
    ...
    Load SkyServer configuration:
      ...
      3) Apply environment variable overrides:
         ...
         - SKYSERVER_BASE_PATH → config["base_path"]
      4) Normalize:
         - SIM_MODE → bool (default True if missing)
         - base_path → absolute, expanded, resolved
