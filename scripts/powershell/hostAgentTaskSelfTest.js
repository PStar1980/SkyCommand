...
assert.match(taskScript, /scheduled-task\.runner\.pid/);
assert.match(taskScript, /function Get-RecordedHostAgentRunnerProcess/);
assert.match(taskScript, /function Stop-ValidatedHostAgentRunnerTree/);
assert.match(taskScript, /function Stop-HostAgentScheduledRuntime/);
assert.match(taskScript, /taskkill\.exe/);
assert.match(taskScript, /\/PID/);
...
assert.match(runnerScript, /scheduled-task\.runner\.pid/);
assert.match(runnerScript, /WriteAllText\(\$runnerPidPath, \[string\]\$PID\)/);
assert.match(runnerScript, /runner PID \$PID/);
...
assert.match(runnerScript, /function Hide-HostAgentConsoleWindow/);
...
assert.match(taskScript, /0x00041306 \/ 267014 is SCHED_S_TASK_TERMINATED/);
assert.match(taskScript, /STOPPED \(TASK SCHEDULER READY\)/);
assert.match(taskScript, /intentionally stopped/);
