...
function getWorkspaceRoot() {
  ...
  if (!configured) {
    fail(
      'SKYCOMMAND_DOCKER_WORKSPACE_ROOT is required before starting the Docker Node worker. Use the host path to the SkyEco System folder.',
    );
  }
...
function getDockerGitConfiguration() {
  ...
  if (!tokenFile) {
    fail(
      'SKYCOMMAND_DOCKER_GITHUB_TOKEN_FILE is required when SKYCOMMAND_DOCKER_GIT_ENABLED=true.',
    );
  }
...
function main() {
  ...
  switch (action) {
    case 'up':
      runCompose(['up', '-d', '--build', 'node-worker']);
      break;
    case 'restart':
      runCompose(['up', '-d', '--build', '--force-recreate', 'node-worker']);
      break;
    case 'stack-up':
      runCompose(['up', '-d', '--build', 'temporal', 'temporal-worker', 'node-worker']);
      break;
    case 'git-check':
