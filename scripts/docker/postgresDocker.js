...
const {
  ...
  readRepositoryEnv,
  upsertEnv,
} = require('../../packages/core/src/repositoryEnvironment');
...
function normalizeDatabaseName(value) {
  ...
  if (!/^[A-Za-z][A-Za-z0-9_]{0,62}$/.test(databaseName)) {
    fail(
      `PGDATABASE must be a safe PostgreSQL database name. Received: ${databaseName || '<blank>'}`,
    );
  }
...
function getSourcePort() {
  const raw = String(
    process.env.SKYCOMMAND_POSTGRES_SOURCE_PORT || process.env.PGPORT || '5432',
  ).trim();
...
function getCandidatePort() {
  ...
  if (port === getSourcePort()) {
    fail(
      'The Docker PostgreSQL host port must differ from the Windows source port so blue/green rollback remains available during cutover.',
    );
  }
...
function prepareEnvironment() {
  readRepositoryEnv(repositoryRoot);
  if (!process.env.PGPASSWORD)
    fail('PGPASSWORD is required for PostgreSQL Docker migration operations.');
...
function ensureCandidateUp({ recreate = false } = {}) {
  ...
  console.log(
    `[SkyCommand PostgreSQL] source=postgresql://127.0.0.1:${getSourcePort()}/${process.env.PGDATABASE}`,
  );
}
...
function dumpDatabase({ label, command }) {
  ...
  const backupPath = path.join(
    backupDirectory,
    `${databaseName}_${timestampForFilename()}_${label}.dump`,
  );
  ...
  const result = spawnSync(
    'docker',
    ['compose', 'exec', '-T', 'postgres', 'bash', '-lc', command],
    {
      cwd: repositoryRoot,
      env: process.env,
...
function restoreBackup(backupPath) {
  ...
  console.log(
    '[SkyCommand PostgreSQL] Recreating candidate database with the Docker cluster locale.',
  );
  runCompose([
    'exec',
    '-T',
...
function assertNoActiveSourceRuns() {
  const active = Number.parseInt(
    querySourceScalar(
      "SELECT COUNT(*) FROM worker.workflow_run_records WHERE status IN ('QUEUED','RUNNING')",
    ),
    10,
  );
  if (!Number.isInteger(active))
    fail('Could not determine active workflow count on the Windows PostgreSQL source.');
  if (active > 0)
    fail(
      `Refusing database cutover while ${active} workflow run(s) are QUEUED/RUNNING on the source database.`,
    );
...
function stopApplicationWriters() {
  console.log(
    '[SkyCommand PostgreSQL] Quiescing Web/API/Node-worker/Temporal-worker before the final source snapshot.',
  );
...
function writeRuntimeCutoverEnvironment() {
  ...
  const databaseName = normalizeDatabaseName(fileEnvironment.PGDATABASE || process.env.PGDATABASE);
  const backupPath = path.join(
    getBackupDirectory(),
    `.env_${timestampForFilename()}_pre_postgres_cutover`,
  );
  ...
  console.log(
    `[SkyCommand PostgreSQL] Runtime environment switched to Docker PostgreSQL. Previous .env preserved at ${backupPath}`,
  );
  return backupPath;
}
...
function restoreEnvironmentBackup(envBackupPath) {
  if (!envBackupPath || !fs.existsSync(envBackupPath))
    fail(`Cutover .env backup does not exist: ${envBackupPath || '<blank>'}`);
...
function stageShadowDatabase() {
  ...
  console.log(
    '[SkyCommand PostgreSQL] Shadow database staged and critical tool/workflow parity passed.',
  );
  console.log(
    '[SkyCommand PostgreSQL] Existing Docker application services still point to the Windows PostgreSQL source. No cutover has occurred.',
  );
}
...
function cutover() {
  ...
  ensureCandidateUp();
  console.log(
    '[SkyCommand PostgreSQL] Prebuilding application images before the write freeze to minimize cutover downtime.',
  );
  ...
  try {
    ...
    console.log(
      '[SkyCommand PostgreSQL] Starting the complete Docker runtime against postgres:5432.',
    );
    startApplicationStack();
    runNode('scripts/docker/postgresCutoverCheck.js');

    console.log(
      '[SkyCommand PostgreSQL] CUTOVER COMPLETE. Docker PostgreSQL is now authoritative for SkyCommand.',
    );
    console.log(`[SkyCommand PostgreSQL] Host tools/CLI now use 127.0.0.1:${getCandidatePort()}.`);
    console.log(
      `[SkyCommand PostgreSQL] Windows PostgreSQL on port ${getSourcePort()} remains online only as a short-lived rollback fallback. Do not write to it.`,
    );
  } catch (error) {
    console.error(`[SkyCommand PostgreSQL] Cutover failed: ${error.message}`);
    if (envBackupPath) {
      console.error(
        '[SkyCommand PostgreSQL] Restoring the pre-cutover runtime environment and source-backed Docker services.',
      );
...
function rollback() {
  const marker = loadCutoverMarker();
  console.log(
    '[SkyCommand PostgreSQL] WARNING: rollback does not copy post-cutover Docker writes back to Windows PostgreSQL. Use only for immediate cutover recovery.',
  );
  stopApplicationWriters();
  ...
  console.log(
    `[SkyCommand PostgreSQL] Runtime rolled back to the Windows PostgreSQL source on port ${marker.sourcePort}.`,
  );
}

function persistenceProof() {
  if (!isDockerRuntimeActive())
    fail('Persistence proof requires an active Docker PostgreSQL cutover.');
  runNode('scripts/docker/postgresCutoverCheck.js');
  const backupPath = createCandidateBackup();
  console.log(
    '[SkyCommand PostgreSQL] Cold-stopping the full Docker runtime, including PostgreSQL.',
  );
  runCompose([
    'stop',
    'web',
  ...
  console.log(
    '[SkyCommand PostgreSQL] Restarting the full Docker runtime from persistent volumes.',
  );
  startApplicationStack();
  runNode('scripts/docker/postgresCutoverCheck.js');
  const marker = loadCutoverMarker();
  writeCutoverMarker({
    ...
    persistenceProvenAt: new Date().toISOString(),
    dockerBackupPath: backupPath,
  });
  console.log(
    `[SkyCommand PostgreSQL] Persistence proof passed. Docker baseline backup: ${backupPath}`,
  );
}
...
function finalize() {
  ...
  console.log(
    `[SkyCommand PostgreSQL] The Windows PostgreSQL service on port ${marker.sourcePort} can now be stopped/disabled after your final UI/CLI acceptance check.`,
  );
}
...
function main() {
  ...
  switch (action) {
    ...
    default:
      fail(
        `Unsupported action '${action}'. Supported actions: up, restart, backup, stage, cutover, rollback, persistence, finalize.`,
      );
  }
}
...
module.exports = {
  ...
  upsertEnv,
  writeRuntimeCutoverEnvironment,
};
