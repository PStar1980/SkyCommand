...
function getWorkspaceRoot() {
  ...
  if (!configured) {
    fail(
      'SKYCOMMAND_DOCKER_WORKSPACE_ROOT is required before starting the Docker API. Use the host path to the SkyEco System folder.',
    );
  }
...
function getDockerGitConfiguration() {
  ...
  if (!tokenFile) {
    fail('SKYCOMMAND_DOCKER_GITHUB_TOKEN_FILE is required when SKYCOMMAND_DOCKER_GIT_ENABLED=true.');
  }
...
function main() {
  ...
  switch (action) {
    ...
    case 'git-check':
      ...
      runCompose(['exec', '-T', 'api', 'node', 'scripts/docker/temporalWorkerGitCheck.js']);
      break;
    default:
